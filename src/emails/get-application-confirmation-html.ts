/**
 * Application confirmation email HTML (sent when someone submits the patient application).
 *
 * To change the design: edit the string below or replace with your full HTML.
 * Placeholders (replaced at send time):
 *   __FIRST_NAME__       - Patient first name
 *   __SCHEDULING_LINK__  - Link to schedule consultation (tracking + calendar)
 *   __PROGRAM_BROCHURE_URL__ - Link for "Download Program Brochure" (e.g. program overview PDF)
 *   __CONTACT_EMAIL__    - Contact email (e.g. contactus@theibogainstitute.org)
 *   __FILLER_MESSAGE__       - Optional HTML when application was submitted on behalf (otherwise empty)
 *   __CREDENTIALS_SECTION__  - Optional HTML with portal login credentials (when new account created; otherwise empty)
 */
...
/** Build HTML for the application confirmation email. */
export function getApplicationConfirmationHtml(params: ApplicationConfirmationParams): string {
  return APPLICATION_CONFIRMATION_TEMPLATE.replace(/__FIRST_NAME__/g, params.firstName)
    ...
    .replace(/__FILLER_MESSAGE__/g, params.fillerMessage)
    .replace(/__CREDENTIALS_SECTION__/g, params.credentialsSection)
}

// Template: design from Postcards (Instrument Serif, Inter, banner, CTAs, contact, footer).
// Replace this constant with your full HTML; keep the __PLACEHOLDER__ tokens for dynamic values.
const APPLICATION_CONFIRMATION_TEMPLATE = `<!DOCTYPE html>
...
body{margin:0;padding:0;background:#ece9df;font-family:'Inter',Arial,sans-serif;-webkit-font-smoothing:antialiased;-webkit-text-size-adjust:100%;} table{border-collapse:collapse;}
.banner-heading{font-family:'Instrument Serif',Georgia,serif;}
@media (max-width:620px){
  .email-body p,.email-body li,.email-body span{font-size:19px !important;}
  .email-body p[style*="font-size:19px"],.email-body .cred-heading{font-size:24px !important;}
  table[role="presentation"]{max-width:100% !important;}
  .email-banner-title{font-size:36px !important;line-height:1.2 !important;}
  .email-cta-section p{font-size:22px !important;}
  .email-cta-section p:last-of-type{font-size:18px !important;}
  .credentials-block .cred-heading{font-size:24px !important;}
  .credentials-block .cred-body,.credentials-block .cred-text{font-size:20px !important;line-height:1.5 !important;}
  .credentials-block .cred-note{font-size:18px !important;}
  .credentials-block .cred-cta{font-size:20px !important;padding:12px 28px !important;}
  .email-footer .email-footer-links td{text-align:center !important;}
  .email-footer .email-footer-links table table{margin-left:auto !important;margin-right:auto !important;}
  .email-footer .email-footer-links table table td{display:inline-block !important;width:auto !important;}
  .email-footer .email-footer-social td{text-align:center !important;}
  .email-footer a{font-size:18px !important;}
}
</style>
</head>
...
<div class="banner-heading email-banner-title" style="font-family:'Instrument Serif',Georgia,serif;font-size:50px;line-height:130%;color:#fff;letter-spacing:-0.03em">Your Wellness Journey Begins</div>
</td></tr></table>
</td></tr></table>
</td></tr>
...
<tr><td class="email-body" style="padding:48px">
...
<tr><td class="email-cta-section" style="padding:48px;background-color:#6e7a46;border-radius:0" bgcolor="#6e7a46">
...
<tr><td class="email-footer" style="padding:48px;background-color:#272315;border-radius:0 0 10px 10px" bgcolor="#272315">
