  ...
  async function onSubmit(data: MedicalIntakeReportInput) {
    ...
    try {
      ...
      const result = initialData 
        ? await updateMedicalIntakeReport({
            ...submitData,
            is_completed: true,
          } as any)
        : await submitMedicalIntakeReport(submitData as any)
      
      if (result?.data?.success) {
        toast.success(initialData ? 'Medical intake report updated successfully' : 'Medical intake report submitted successfully')
        onSuccess?.()
      } else {
